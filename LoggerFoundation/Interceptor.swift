...
///     func intercept(level: Logger.Level, message: String, error: (any Error)?, data: [String: String]?,
...
///
/// // ...and stop it receiving logs again
/// Logger.remove(token)
///
...
public protocol Interceptor: Sendable {
    ...
    ///
    /// - Note: Called on the logger's serial queue, never concurrently with itself, and in the
    ///   order the logs were recorded. Conformers are `Sendable` because of it.
    func intercept(
        level: Logger.Level,
        message: String,
        error: (any Error)?,
